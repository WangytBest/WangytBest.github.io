...
如果`Origin`指定的源，不在许可范围内，服务器会返回一个正常的`HTTP`回应。浏览器发现，这个回应的头信息没有包含`Access-Control-Allow-Origin`字段，就知道出错了，从而抛出一个错误，被`XMLHttpRequest`的`onerror`回调函数捕获。注意，这种错误无法通过状态码识别，因为HTTP回应的状态码有可能是200。
...
CORS请求默认不发送`Cookie`和`HTTP认证信息`。如果要把Cookie发到服务器，一方面要服务器同意，指`Access-Control-Allow-Credentials`字段。`Access-Control-Allow-Credentials: true`
